            ...
            case "object":
                ...
                if (_class === ARRAY_CLASS) {
                    ...
                    for (var i = 0, length = keys.length; i < length; i++) {
                        var key =
                            keys[i], str = Str(key, value, stack);
...
Object.extend(Function.prototype, function () {
    ...
    function update(array, prop) {
        var arrayLength = array.length, length = prop.length;
        while (length--)array[arrayLength + length] = prop[length];
        return array
    }

    function merge(array, prop) {
        array = slice.call(array, 0);
        return update(array, prop)
    }
    ...
    function bind(context) {
        ...
        var __method = this, prop = slice.call(arguments, 1);
        return function () {
            var a = merge(prop, arguments);
    ...
    function bindAsEventListener(context) {
        ...
        return function (event) {
            var a = update([event || window.event], prop);
            return __method.apply(context, a)
        }
    }
    ...
    function curry() {
        ...
        var __method = this, prop = slice.call(arguments,
            0);
        return function () {
            var a = merge(prop, arguments);
    ...
    function delay(timeout) {
        ...
        return window.setTimeout(function () {
            return __method.apply(__method, prop)
        }, timeout)
    }

    function defer() {
        var prop = update([0.01], arguments);
        return this.delay.apply(this, prop)
    }
    ...
    function toQueryParams(separator) {
        ...
        return match[1].split(separator || "&").inject({}, function (hash, pair) {
            if ((pair = pair.split("="))[0]) {
                ...
                if (key in hash) {
                    if (!Object.isArray(hash[key]))hash[key] =
                        [hash[key]];
                    hash[key].push(value)
                } else hash[key] = value
            }
...
var Enumerable = function () {
    ...
    function inject(memo, iterator, context) {
        this.each(function (value, index) {
            memo =
                iterator.call(context, memo, value, index)
        });
        return memo
    }

    function invoke(method) {
        var prop = $A(arguments).slice(1);
        return this.map(function (value) {
            return value[method].apply(value, prop)
        })
    }
    ...
    function zip() {
        var iterator = Prototype.K, prop = $A(arguments);
        if (Object.isFunction(prop.last()))iterator = prop.pop();
        var collections = [this].concat(prop).map($A);
        return this.map(function (value,
                                  index) {
...
(function () {
    ...
    function concat() {
        ...
        length; i++) {
            item = arguments[i];
            if (Object.isArray(item) && !("callee"in item))for (var j = 0, arrayLength = item.length; j < arrayLength; j++)array.push(item[j]); else array.push(item)
        }
        return array
    }
...
Object.extend(Number.prototype, function () {
    ...
    return {
        ...
        succ: succ, times: times, toPaddedString: toPaddedString, abs: abs, round: round, ceil: ceil, floor: floor
    }
}());
    ...
    _complete: false, initialize: function ($super, url, options) {
        ...
        if (state == "Complete") {
            try {
                ...
                (this.options["on" + (this.success() ? "Success" : "Failure")] || Prototype.emptyFunction))(response, response.headerJSON)
            } catch (e) {
                this.dispatchException(e)
            }
...
Ajax.Updater = Class.create(Ajax.Request, {
    initialize: function ($super, container, url, options) {
        ...
        var receiver = this.container[this.success() ?
            "success" : "failure"], options = this.options;
...
if (Prototype.BrowserFeatures.XPath)document._getElementsByXPath = function (expression, parentElement) {
    var results = [];
...
Element.Methods = {
    visible: function (element) {
        ...
        function update(element, content) {
            ...
            if (ANY_INNERHTML_BUGGY)if (tagName in Element._insertionTranslations.tags) {
                ...
                while (element.firstChild)element.removeChild(element.firstChild);
                var nodes = Element._getContentFromAnonymousElement(tagName, content.stripScripts(),
                    true);
    ...
    wrap: function (element, wrapper, attributes) {
        element = $(element);
        if (Object.isElement(wrapper))$(wrapper).writeAttribute(attributes || {}); else if (Object.isString(wrapper))wrapper = new Element(wrapper, attributes); else wrapper = new Element("div", wrapper);
        ...
        $H({"id": "id", "className": "class"}).each(function (pair) {
            var property =
                pair.first(), attribute = pair.last(), value = (element[property] || "").toString();
        ...
        return Element.recursivelyCollect(element,
            "parentNode")
        ...
        if (expression)return Prototype.Selector.find(element.previousSiblings(), expression, index); else return element.recursivelyCollect("previousSibling", index + 1)[index]
    }, next: function (element, expression, index) {
        element = $(element);
        if (Object.isNumber(expression))index = expression, expression = false;
        if (!Object.isNumber(index))index = 0;
        if (expression)return Prototype.Selector.find(element.nextSiblings(), expression, index); else {
        ...
        element = $(element);
        var id = Element.readAttribute(element,
            "id");
        if (id)return id;
    ...
    getWidth: function (element) {
        ...
        if (!(element = $(element)))return;
        if (!Element.hasClassName(element, className))element.className +=
    ...
    setOpacity: function (element, value) {
        element = $(element);
        element.style.opacity = value == 1 || value === "" ? "" : value < 1E-5 ? 0 : value;
        return element
...
if (Prototype.Browser.Opera) {
    ...
    Element._attributeTranslations = function () {
        var classProp =
            "className", forProp = "for", el = document.createElement("div");
            ...
            read: {
                names: {"class": classProp, "className": classProp, "for": forProp, "htmlFor": forProp}, values: {
                    ...
                    _getAttr2: function (element, attribute) {
                        ...
                        if (String(value).indexOf("{") > -1)f = function (element, attribute) {
                        ...
                        }; else if (value === "")f = function (element, attribute) {
                            attribute = element.getAttribute(attribute);
                            if (!attribute)return null;
                            return attribute.strip()
                        };
    ...
    /rv:1\.8\.0/.test(navigator.userAgent))Element.Methods.setOpacity = function (element, value) {
    element = $(element);
    element.style.opacity = value == 1 ? 0.999999 : value === "" ? "" : value < 1E-5 ? 0 : value;
    return element
}; else if (Prototype.Browser.WebKit)Element.Methods.setOpacity = function (element, value) {
    element = $(element);
    element.style.opacity = value == 1 || value === "" ? "" : value < 1E-5 ? 0 : value;
...
if ("outerHTML"in document.documentElement)Element.Methods.replace = function (element, content) {
    element = $(element);
    if (content && content.toElement)content = content.toElement();
    if (Object.isElement(content)) {
        element.parentNode.replaceChild(content, element);
...
if (document.documentElement.hasAttribute)Element.hasAttribute = function (element, attribute) {
...
Element.addMethods = function (methods) {
    ...
    if (!tagName)Object.extend(Element.Methods,
        methods || {}); else if (Object.isArray(tagName))tagName.each(extend); else extend(tagName);
    ...
    function copy(methods, destination, onlyIfAbsent) {
        ...
        for (var property in methods) {
            ...
            if (!onlyIfAbsent || !(property in destination))destination[property] =
                value.methodize()
        }
    }
...
(function () {
    ...
    function getPixelValue(value, property, context) {
        ...
        if (element && isPercentage) {
            ...
            if (context === document.viewport)if (isHorizontal)whole = document.viewport.getWidth(); else {
                if (isVertical)whole = document.viewport.getHeight()
            } else if (isHorizontal)whole = $(context).measure("width"); else if (isVertical)whole = $(context).measure("height");
    ...
    Element.Layout = Class.create(Hash, {
        initialize: function ($super, element, preCompute) {
            ...
            if (width && positionedWidth === width)newWidth = getPixelValue(element, "width", context); else if (position === "absolute" || position === "fixed")newWidth = getPixelValue(element, "width", context); else {
                ...
                newWidth = pLayout.get("width") - this.get("margin-left") - this.get("border-left") -
                    this.get("padding-left") - this.get("padding-right") - this.get("border-right") - this.get("margin-right")
            }
            ...
            var prop = $A(arguments);
            var keys = prop.length === 0 ? Element.Layout.PROPERTIES : prop.join(" ").split(" ");
            var obj = {};
            ...
            var prop = $A(arguments);
            var keys = prop.length === 0 ?
                Element.Layout.PROPERTIES : prop.join(" ").split(" ");
            var css = {};
            keys.each(function (key) {
                if (!Element.Layout.PROPERTIES.include(key))return;
    ...
    Object.extend(Element.Layout, {
        ...
        COMPUTATIONS: {
            "height": function (element) {
                ...
                var bHeight =
                    this.get("border-box-height"), mTop = this.get("margin-top"), mBottom = this.get("margin-bottom");
    ...
    function getOffsetParent(element) {
        ...
        if (isDocument(element) ||
            (isDetached(element) || (isBody(element) || isHtml(element))))return $(document.body);
    ...
    function viewportOffset(forElement) {
        ...
        do {
            valueT += element.offsetTop || 0;
            valueL += element.offsetLeft || 0;
            if (element.offsetParent == docBody && Element.getStyle(element, "position") ==
                "absolute")break
    ...
    } else if (Prototype.Browser.Webkit)cumulativeOffset =
        function (element) {
            element = $(element);
            var valueT = 0, valueL = 0;
            do {
...
(function () {
    ...
    var Sizzle = function (selector, context, results, seed) {
        ...
        if (parts.length > 1 && origPOS.exec(selector))if (parts.length === 2 && Expr.relative[parts[0]])set = posProcess(parts[0] + parts[1], context); else {
            ...
            if (context) {
                ...
                set = ret.expr ?
                    Sizzle.filter(ret.expr, ret.set) : ret.set;
        ...
        if (toString.call(checkSet) === "[object Array]")if (!prune)results.push.apply(results, checkSet); else if (context &&
            context.nodeType === 1)for (var i = 0; checkSet[i] != null; i++) {
    ...
    Sizzle.filter = function (expr, set, inplace, not) {
        ...
        while (expr && set.length) {
            ...
            if (expr == old)if (anyFound == null)throw"Syntax error, unrecognized expression: " +
            expr; else break;
            old = expr
        }
        return curLoop
    ...
    var Expr = Sizzle.selectors = {
        ...
        relative: {
            "+": function (checkSet, part, isXML) {
                ...
                for (var i = 0, l = checkSet.length, elem; i < l; i++)if (elem = checkSet[i]) {
                    ...
                    checkSet[i] = isPartStrNotTag || elem && elem.nodeName === part ? elem || false :
                    elem === part
                }
                ...
                if (isPartStr && !/\W/.test(part)) {
                    ...
                    if (isPartStr)Sizzle.filter(part,
                        checkSet, true)
                }
            }, "": function (checkSet, part, isXML) {
        ...
        preFilter: {
            CLASS: function (match,
                ...
                if (match[1] ===
                    "not")if ((chunker.exec(match[3]) || "").length > 1 || /^\w/.test(match[3]))match[3] = Sizzle(match[3], null, null, curLoop); else {
        ...
        filter: {
            PSEUDO: function (elem, match, i, array) {
                ...
                if (filter)return filter(elem, i, match, array); else if (name === "contains")return (elem.textContent || (elem.innerText || "")).indexOf(match[3]) >=
                    0; else if (name === "not") {
                    var not = match[3];
                ...
                return result == null ? type === "!=" : type === "=" ? value === check : type === "*=" ? value.indexOf(check) >= 0 : type === "~=" ? (" " + value + " ").indexOf(check) >= 0 : !check ? value && result !== false : type === "!=" ? value != check : type === "^=" ? value.indexOf(check) ===
                0 : type === "$=" ? value.substr(value.length - check.length) === check : type === "|=" ? value === check || value.substr(0, check.length + 1) === check + "-" : false
    ...
    if (document.documentElement.compareDocumentPosition)sortOrder = function (a, b) {
        ...
        if (ret === 0)hasDuplicate = true;
        return ret
    }; else if (document.createRange)sortOrder =
        function (a, b) {
...
(function () {
    ...
    if (window.attachEvent)if (!window.addEventListener)_isButton =
        _isButtonForLegacyEvents; else _isButton = function (event, code) {
        return isIELegacyEvent(event) ? _isButtonForLegacyEvents(event, code) : _isButtonForDOMEvents(event, code)
    }; else if (Prototype.Browser.WebKit)_isButton = _isButtonForWebKit; else _isButton = _isButtonForDOMEvents;
    ...
    function element(event) {
        ...
        var node = event.target, type =
            event.type, currentTarget = event.currentTarget;
    ...
    function _createResponder(element, eventName, handler) {
        ...
        var responder;
        if (eventName.include(":"))responder = function (event) {
            ...
            if (eventName === "mouseenter" || eventName === "mouseleave")responder = function (event) {
                Event.extend(event, element);
    ...
    if (!MOUSEENTER_MOUSELEAVE_EVENTS_SUPPORTED)_getDOMEventName =
        function (eventName) {
    ...
    function observe(element, eventName, handler) {
        ...
        if (eventName.include(":"))if (element.addEventListener)element.addEventListener("dataavailable", responder, false); else {
            element.attachEvent("ondataavailable", responder);
            element.attachEvent("onlosecapture", responder)
        } else {
            var actualEventName = _getDOMEventName(eventName);
            if (element.addEventListener)element.addEventListener(actualEventName,
                responder, false); else element.attachEvent("on" + actualEventName, responder)
        }
        return element
    }
    ...
    function stopObserving(element, eventName, handler) {
        ...
        if (!responder)return element;
        if (eventName.include(":"))if (element.removeEventListener)element.removeEventListener("dataavailable", responder, false); else {
            element.detachEvent("ondataavailable", responder);
            element.detachEvent("onlosecapture", responder)
        } else {
            var actualEventName = _getDOMEventName(eventName);
            if (element.removeEventListener)element.removeEventListener(actualEventName,
                responder, false); else element.detachEvent("on" + actualEventName, responder)
        }
    ...
    function fire(element, eventName, memo, bubble) {
        ...
        event.memo = memo || {};
        if (document.createEvent)element.dispatchEvent(event); else element.fireEvent(event.eventType, event);
    ...
    Event.Handler = Class.create({
        initialize: function (element, eventName, selector, callback) {
            ...
            Event.observe(this.element, this.eventName,
                this.handler);
            return this
        }, stop: function () {
